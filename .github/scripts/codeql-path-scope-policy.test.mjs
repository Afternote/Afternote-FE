...
test("CodeQL classifies every PR file and defaults to full analysis", async () => {
    const source = await readFile(workflowUrl, "utf8");
    ...
    assert.match(classifier, /docs_only: \$\{\{ steps\.path-scope\.outputs\.docs_only \|\| 'false' \}\}/);
...
test("CodeQL preserves both required context names and skips only after a successful docs-only classification", async () => {
    const source = await readFile(workflowUrl, "utf8");
    const actions = jobBlock(source, "analyze-actions", "analyze-java-kotlin");
    const kotlin = jobBlock(source, "analyze-java-kotlin");
    const failClosed = /if: \$\{\{ !cancelled\(\) && \(needs\.classify-changes\.result != 'success' \|\| needs\.classify-changes\.outputs\.docs_only != 'true'\) \}\}/;
    ...
    assert.match(actions, failClosed);
    assert.match(kotlin, failClosed);

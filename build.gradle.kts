...
// 클래스패스의 보안 하한(#921·#975~#985·#1072). 상류가 취약 버전을 물고 있고 상류 최신판도
...
// 4.1.110.Final)·AGP androidLintTool 과 UTP(httpclient 4.5.6)·ktlint CLI(logback 1.3.16)·Kakao
// SDK 2.23.2(okhttp 4.9.2→okio 2.8.0). androidLintTool 처럼 AGP 가 뒤늦게 만드는 configuration
// 까지 잡도록 configureEach 로 걸고, require 시맨틱이라 상류가 하한 이상을 선언하게 되면 그쪽이
// 이긴다. okhttp 를 뺀 나머지는 빌드 도구 경유라 APK 산출물에는 없다(releaseRuntimeClasspath 실측).
// build-logic 은 별도 빌드라 build-logic/build.gradle.kts 가 같은 하한을 선언한다.

 ...
 * ## 여기 없는 목적지
 *
 * 애프터노트 상세(`/afternote/{id}`)는 아직 이 표에 없다. 그 화면은 애프터노트 피처의 로컬
 * Navigation 3 스택 안에 있고 그 스택의 시작점이 지문 관문으로 고정돼 있어, 밖에서 상세를
 * 지목해 들어갈 인자가 없다. 도착할 자리가 없는 경로를 계약에만 실으면 링크는 앱을 열되

 ...
 * 서버가 새 경로를 먼저 배포했는지, 남이 우리 도메인을 흉내 냈는지는 이유가 남아야 갈린다.
 *
 * [reportValue] 는 리포팅 속성에 싣는 고정 문자열이다. enum 이름을 바꿔도 이 값은 유지해야 한다 —
 * 대시보드 집계가 이 값으로 쌓인다.
 *
 * **원본 링크 자체는 절대 싣지 않는다.** 거절된 링크는 계약 밖 입력이라 `/timeletter/9` 처럼 ID 가
 * 실린 경로나 query 가 그대로 들어 있을 수 있고, 그대로 로그에 남기면 식별 가능한 사용자 자료가
 * 리포팅으로 새 나간다. 관측에 쓰는 값은 이 이유뿐이다.
 */

...
/**
 * 한 칸 내린다 — **바닥이면 스택을 비우지 않고** [navigationCallbacks] 에 넘긴다.
 *
 ...
 */
public fun NavBackStack<NavKey>.popOrExit(navigationCallbacks: FeatureNavigationCallbacks) {
    if (size > 1) {
        removeAt(lastIndex)
    } else {
        navigationCallbacks.exit()
    }
}

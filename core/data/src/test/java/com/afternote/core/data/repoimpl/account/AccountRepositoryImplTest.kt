 ...
 *
 * 설정의 비밀번호 변경(#564)도 같은 규칙을 따른다 — 이 메서드만 번역이 빠져 있어 1206·1702 가
 * 원본 `ApiException` 으로 새고 있었다. 화면이 타입으로만 분기하므로 번역이 끊기면 조용히 폴백
 * 문구가 나간다.
 *
...
class AccountRepositoryImplTest {
    ...
    @Test
    fun `passwordChange - 기존과 같은 비밀번호(1206)는 PasswordUnchanged 로 번역`() {
        val repository =
            repository(
                FakeAccountApiService(
                    onPasswordChange = {
        ...
        val result = runBlocking { repository.passwordChange(CURRENT_PASSWORD, NEW_PASSWORD) }

        assertTrue(result.exceptionOrNull() is CoreAuthFailure.PasswordUnchanged)
    }

    @Test
    fun `passwordChange - 소셜 가입 계정(1702)은 SocialSignUpAccount 로 번역`() {
        val repository =
            repository(
                FakeAccountApiService(
                    onPasswordChange = {
    ...
    @Test
    fun `passwordChange - 그 외 code 실패는 원본 ApiException 유지 (폴백 문구 경로)`() {
        val repository =
            repository(
                FakeAccountApiService(
                    onPasswordChange = {
                        throw ApiException(
                            ...
                            code = 1202,
                            serverMessage = null,
                            fallbackMessage = "아이디 또는 비밀번호가 일치하지 않습니다.",
                        )
                    },
                ),
            )
        ...
        val error = result.exceptionOrNull()
        assertTrue(error is ApiException)
        assertEquals(1202, (error as ApiException).code)
    }

    @Test
    fun `passwordChange - 두 필드를 서버 요청 그대로 싣는다`() {
        var sent: PasswordChangeRequestDto? = null
        val repository =
            repository(
                FakeAccountApiService(
                    onPasswordChange = { body ->
                        sent = body
                        success(Unit)
                    },
                ),
        ...
        assertTrue(result.isSuccess)
        assertEquals(PasswordChangeRequestDto(CURRENT_PASSWORD, NEW_PASSWORD), sent)
    }
...
private class FakeAccountApiService(
    ...
    private val onPasswordChange: (PasswordChangeRequestDto) -> BaseResponse<Unit> = {
        error("passwordChange 는 이 시나리오에서 호출되면 안 됨")
    },
) : AccountApiService {
    ...
    override suspend fun passwordChange(body: PasswordChangeRequestDto): BaseResponse<Unit> = onPasswordChange(body)
}

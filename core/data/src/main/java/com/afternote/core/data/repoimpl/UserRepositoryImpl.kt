 ...
 *
 * 수신자는 [UserReceiverRepositoryImpl], 서버 정본 프로필은 [MyProfileRepositoryImpl] 이 소유하고
 * 여기서는 위임만 한다. 두 구현을 Hilt 가 아니라 이 생성자가 직접 조립하는 이유는 단일 인스턴스
 * 보장이다 — `UserRepository` 로 등록한 수신자가 `UserReceiverRepository` 구독자의 목록도 갱신해야 한다.
 *
 * 남은 계정·푸시 6멤버는 core 에 좁은 계약을 신설하지 않고 `feature:setting` 으로 곧장 내린다 (#1429).
 *
 * `app` 의 androidTest(ReceiverSessionIsolationAndroidTest)와 `feature:setting` 테스트가 Hilt 를
 * 우회해 이 구현을 직접 조립하므로 형제 구현체들과 달리 `internal` 로 닫지 못하고, 같은 이유로
 * 생성자 시그니처도 그대로 둔다. 닫으려면 그 테스트들의 페이크 기반 개조가 선행돼야 한다 (#930).
 */
class UserRepositoryImpl
    ...
    constructor(
        ...
        UserReceiverRepository by UserReceiverRepositoryImpl(userApiService, authRepository, errorReporter),
        MyProfileRepository by MyProfileRepositoryImpl(userApiService) {
        /**

 ...
 *
 * 와이어는 누락·미래 값을 받아들이되 도메인에는 임의의 종류를 채우지 않는다. 그래야 필터·아이콘·
 * 상세 라우팅이 다른 종류로 조용히 왜곡되지 않는다.
 */
fun ReceivedAfternoteDto.toDomainOrNull(): AfterNoteListItem? {
    val resolvedType = category?.let(::afternoteTypeFromServerCategory) ?: return null
...
fun ReceivedAfternoteListDto.toDomainResult(errorReporter: ErrorReporter): AfterNotesListResult =
    AfterNotesListResult(
        items = afternotes.toReceiverDomainList(errorReporter),

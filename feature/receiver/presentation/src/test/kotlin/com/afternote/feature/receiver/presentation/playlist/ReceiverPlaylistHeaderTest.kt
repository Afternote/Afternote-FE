...
class ReceiverPlaylistHeaderTest {
    ...
    fun `수신자 플레이리스트에는 곡 검색 입력이 없다`() {
        ...
        val searchLabel = composeRule.activity.getString(AfternoteR.string.song_search_label)
        val searchPlaceholder = composeRule.activity.getString(AfternoteR.string.song_search_placeholder)
    ...
    fun `수신자 플레이리스트 헤더는 곡 수를 보여준다`() {
        ...
        val songCount = composeRule.activity.getString(ReceiverR.string.receiver_playlist_song_count_format, songs.size)

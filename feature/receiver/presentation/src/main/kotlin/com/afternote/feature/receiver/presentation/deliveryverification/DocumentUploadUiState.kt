 ...
 */
@Immutable
internal data class DocumentUploadUiState(
     ...
     * 한 번 표시한 뒤 `DocumentUploadIntent.ConsumeError` 로 되돌리므로, 실패한 적 없는 상태와 이미 보여 준 상태가 같은
    ...
    /** 제출 성공 신호 — UI 가 LaunchedEffect 로 완료 화면 이동 후 [DocumentUploadIntent.ConsumeSubmitted] 로 reset. */
 ...
 */
@Immutable
internal data class DocumentSlotState(
    val displayName: String? = null,
    val fileUrl: String? = null,
    val isUploading: Boolean = false,
)

internal enum class DocumentSlot {
    DeathCertificate,

...
sealed class ReceiverFailure(
    ...
    /**
     * 서버가 응답을 내려주며 거절한 실패 중 **사유를 따로 가르지 않은 것**.
     *
     * 흐름별로 타입을 나누지 않는다 — 어느 endpoint 가 거절했는지는 stack trace 에 남고, 타입으로
     * 갈라 보던 프로덕션 코드가 없었다(#934 실측). 화면 처리가 실제로 달라지는 사유만
     * [DeliveryConditionNotMet] 처럼 별도 타입으로 올린다.
     *
     ...
     *   이 값을 소비처가 보는 구조 자체를 Data 계층으로 내리는 것은 #1053 범위다.

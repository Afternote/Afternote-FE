 ...
 *
 * 소셜 로그인 콜백만 파라미터로 남는다. 토큰을 받아오는 일이 Activity·CredentialManager
 * 의존이라 [LoginScreen] 이 하고, ViewModel 에는 문자열만 Intent 로 들어간다.
 */
...
internal fun LoginContent(
    ...
    LaunchedEffect(emailState) {
        snapshotFlow { emailState.text.toString() }.collect { onIntent(LoginIntent.UpdateEmail(it)) }
    }
    LaunchedEffect(passwordState) {
        snapshotFlow { passwordState.text.toString() }.collect { onIntent(LoginIntent.UpdatePassword(it)) }
    }
    Scaffold(
        modifier = modifier,

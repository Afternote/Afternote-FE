...
class ReceiverSavePolicyTest {
    ...
    /**
     * 옮겨 오기 전 ViewModel 은 `runCatchingCancellable` 로 감싸 `Exception` 이 아닌 `Throwable` 도
     * 화면 오류로 바꿨다. UseCase 가 `Exception` 만 잡으면 그 경로가 코루틴 밖으로 튀어 화면이 저장 중에
     * 머문다 — 포착 범위를 좁히는 회귀를 여기서 막는다.
     */
    @Test
    fun `basic info failure keeps non exception throwable as cause`() =
        runTest {
            val error = AssertionError("save failed")
            val repository = FakeUserReceiverRepository(onUpdateReceiver = { _, _, _, _, _ -> throw error })
            val result = save(repository)
            assertSame(error, (result as UpdateReceiverInfoResult.BasicInfoFailed).cause)
            assertTrue(repository.receiverMessageCalls.isEmpty())
    ...
    /** 인사말 쪽도 같은 이유로 `Throwable` 까지 부분 저장 결과로 접는다. */
    @Test
    fun `message failure reports partial save for non exception throwable`() =
        runTest {
            val repository =
                FakeUserReceiverRepository(
                    onUpdateReceiverMessage = { _, _ -> throw AssertionError("message failed") },
                )
            assertEquals(UpdateReceiverInfoResult.MessageFailedAfterBasicInfoUpdated, save(repository))
            assertEquals(1, repository.receiverMessageCalls.size)
        }
    ...
    @Test
    fun `basic info cancellation propagates`() =
        runTest {
            val cancelled = CancellationException("cancelled")
            val repository = FakeUserReceiverRepository(onUpdateReceiver = { _, _, _, _, _ -> throw cancelled })
            try {
                save(repository)

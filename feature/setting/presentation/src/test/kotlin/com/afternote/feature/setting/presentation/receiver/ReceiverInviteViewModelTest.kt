...
class ReceiverInviteViewModelTest {
    ...
    fun `내 이름을 어디서도 못 읽으면 초대를 만들지 않고 실패로 알린다`() =
        runTest(dispatcher) {
            ...
            assertEquals(UiText.Resource(R.string.setting_receiver_invite_sender_name_unavailable), viewModel.uiState.value.errorMessage)
    ...
    fun `발급 실패는 시트를 유지한 채 안내하고 미인증은 로그인 안내다`() =
        runTest(dispatcher) {
            ...
            assertEquals(UiText.Resource(R.string.setting_receiver_invite_create_unauthenticated), viewModel.uiState.value.errorMessage)
            assertEquals("김민서", viewModel.uiState.value.sheetReceiverName)
        }

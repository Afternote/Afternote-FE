 ...
 *
 * 이 VM이 들고 가는 것은 **세션 스코프 UI 초안**뿐이다: 플레이리스트 편집 버퍼(런타임), 에디터 파사드(`AfternoteEditorState`) 참조.
 * 추모 곡 목록의 복원 SSOT는 에디터 `ViewModel`의 `EditorFormState.memorialPlaylistSongs` + `SavedStateHandle` JSON 스냅샷이다.

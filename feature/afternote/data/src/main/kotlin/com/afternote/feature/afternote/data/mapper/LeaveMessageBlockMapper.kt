...
internal fun List<LeaveMessageBlock>.toDtoForCreate(): List<LeaveMessageBlockDto>? =
    map { LeaveMessageBlockDto(title = it.title, body = it.body) }
        .takeIf { it.isNotEmpty() }

/**
 * 도메인 → 수정 요청. **[toDtoForCreate] 처럼 빈 배열을 null 로 접지 않는다** (#1617).
 *
 * 수정은 부분 갱신이라 `null`(안 건드림 → 키 생략)과 `[]`(전부 삭제)가 서로 다른 지시다.
 * 접어 버리면 마지막 말씀을 전부 지운 저장이 「유지」로 흡수돼 서버에 반영되지 않는다.
 */
internal fun List<LeaveMessageBlock>?.toDtoForPatch(): List<LeaveMessageBlockDto>? =

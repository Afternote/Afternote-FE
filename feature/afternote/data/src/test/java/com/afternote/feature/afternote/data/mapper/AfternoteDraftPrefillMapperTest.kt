...
/**
 * [AfternoteDetailDto.toDraftPrefill] 회귀 가드 (임시저장 프리필 = 이어쓰기).
 *
...
class AfternoteDraftPrefillMapperTest {
    ...
    @Test
    fun `toDraftPrefill - 담은 곡과 미디어는 그대로 실린다`() {
        val result =
            AfternoteDetailDto(
                isDraft = true,
                receivers = emptyList(),
    ...
    @Test
    fun `toDraftPrefill - 계정 정보를 아직 안 쓴 임시저장은 credentials 가 null 이다`() {
        val result =
            AfternoteDetailDto(
                isDraft = true,
                receivers = emptyList(),
    ...
    @Test
    fun `toDraftPrefill - 한쪽만 채운 계정 정보는 그 한쪽만 살린다`() {
        val result =
            AfternoteDetailDto(
                isDraft = true,
                receivers = emptyList(),
    ...
    @Test
    fun `toDraftPrefill - 해석할 수 없는 종류는 폼을 못 고르므로 실패한다`() {
        val exception =

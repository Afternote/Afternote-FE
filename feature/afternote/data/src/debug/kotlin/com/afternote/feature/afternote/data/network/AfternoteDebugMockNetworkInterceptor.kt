 ...
 *
 * 고정 응답 본문은 [AfternoteMockFixtures]에서 관리합니다.
 *
...
class AfternoteDebugMockNetworkInterceptor
    ...
    constructor(
            ...
            val json =
                when {
                    path == "/auth/social/login" && method == "POST" -> {
                        AfternoteMockFixtures.MOCK_SOCIAL_LOGIN_JSON
                    }

                    path == "/auth/login" && method == "POST" -> {
                        AfternoteMockFixtures.MOCK_LOGIN_JSON
                    }

                    path == "/auth/logout" && method == "POST" -> {
                        AfternoteMockFixtures.MOCK_LOGOUT_JSON
                    }

                    path == "/music/search" && method == "GET" -> {
                        AfternoteMockFixtures.musicSearchResponseJson
                    }
                    ...
                    path.matches(AFTERNOTE_ID_PATH_REGEX) -> {
                        when (method) {
                            ...
                            "PATCH" -> {
                                AfternoteMockFixtures.MOCK_PATCH_JSON
                            }

                            "DELETE" -> {
                                AfternoteMockFixtures.MOCK_DELETE_JSON
                            }
        ...
        private fun mockDetailJson(path: String): String {
            ...
            return AfternoteMockFixtures.detailJsonForId(id)
        }

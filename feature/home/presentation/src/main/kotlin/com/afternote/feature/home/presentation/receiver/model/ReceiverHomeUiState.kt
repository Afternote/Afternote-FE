 ...
 *
 * [ReceiverDownloadState] 안이 아니라 밖에 둔 것은 가시성 때문이다 — 인터페이스 안의
 * 중첩 선언은 `internal` 을 붙일 수 없고, 이 갈래는 모듈 밖으로 나갈 계약이 아니다.
 */
internal data class FailedWithRetry(
 ...
 * 단계가 따로 있다.
 *
 * 이 갈래를 고르는 것도 그리는 것도 `feature:home:presentation` 안이라 `internal` 이다 —
 * 화면 밖으로 나가는 계약이 아니다 (docs/convention/production-visibility.md).
 */
internal enum class ReceiverDownloadErrorPopup {
